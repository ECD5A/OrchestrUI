...
export async function runExternalLinkCheck(root = ".") {
  ...
  const deferredPrefixes = ["https://github.com/ECD5A/OrchestrUI"];
  const results = [];
  ...
  const deferred = results.filter(({ url, status }) => (status === 0 || status >= 400)
    && deferredPrefixes.some((prefix) => url.startsWith(prefix)));
  const broken = results.filter(({ url, status }) => (status === 404 || status === 410)
    && !deferredPrefixes.some((prefix) => url.startsWith(prefix)));
  const inconclusive = results.filter(({ url, status }) => !deferredPrefixes.some((prefix) => url.startsWith(prefix))
  ...
  for (const result of deferred) {
    console.warn(`DEFERRED ${result.status} ${result.url} — canonical repository URL becomes live at publication`);
  }

  console.log(`External link check: ${entries.length} unique URLs; ${broken.length} broken; ${inconclusive.length} inconclusive; ${deferred.length} publication-deferred.`);
  if (broken.length) process.exitCode = 1;
  return { checked: entries.length, broken: broken.length, inconclusive: inconclusive.length, deferred: deferred.length };
}
