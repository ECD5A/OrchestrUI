...
- [x] Publish `v0.4.0` from a tested commit across GitHub Releases, npm and the MCP Registry
...
The canonical repository is `ECD5A/OrchestrUI`, npm publishes `orchestrui`, and the official MCP Registry identity is `io.github.ECD5A/orchestrui`. Version `0.4.0` is active across all three release surfaces.
